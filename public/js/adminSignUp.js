document.addEventListener("DOMContentLoaded", function () {
  ...
  async function verifyVerificationCode() {
    ...
    try {
      const response = await fetch("/api/admin/verify-verification-code", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async function createAdmin() {
    ...
    try {
      ...
      if (response.ok) {
        showCustomAlert("Admin created successfully");
        // Redirect to admin login page after 3 seconds
        setTimeout(() => {
          window.location.href = "adminLogIn.html";
        }, 3000); // 3-second delay
      } else {
        showCustomAlert(result.message || "Error creating admin.");
      }
    } catch (error) {
      console.error("Error:", error.message);
      showCustomAlert("Error creating admin.");
    }
  }
  ...
  signUpForm.addEventListener("submit", async function (event) {
    ...
    if (isVerified) {
      // If the verification code is valid, create the admin
      createAdmin();
    }
  });
